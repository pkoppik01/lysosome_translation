...
server <- function(input, output, session) {
  ...
  output$ridgeline_plot <- renderPlot({
    ...
    p <- ggplot(df_long, aes(x = value, y = variable, fill = stat(quantile))) +
      stat_density_ridges(quantile_lines = TRUE, geom = "density_ridges_gradient", quantiles = 2,
                          scale = 2, rel_min_height = 0.01, color = "white", lwd = 3) +
      scale_fill_brewer(name = "gray") +
