...
server <- function(input, output, session) {
  ...
  observeEvent(input$button, {
    ...
    # Convert '1' to 'red' and all other values to 'gray' in the color column
    df_filtered <- df_filtered %>%
      mutate(color_col = ifelse(df_filtered[[color_col]] == 1, "red", "gray"))
    
    # Add a new column for alpha values based on the "color_col"
    df_filtered <- df_filtered %>%
      mutate(alpha_col = ifelse(color_col == "red", 0.7, 0.15))
    ...
    volcano_plot <- ggplot(df_filtered, aes_string(x = x_col, y = paste("-log10(", y_col, ")", sep = ""), color = "color_col")) +
      geom_point(size = 3, alpha = 0.7, shape = 21, aes(fill = color_col), stroke = 0.1) +
      scale_fill_manual(values = c("red" = "red", "gray" = "#D3D3D3")) +
