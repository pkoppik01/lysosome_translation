...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sselectInput("x_axis", "X-axis:", choices = NULL, multiple = FALSE),
      selectInput("y_axis", "Y-axis:", choices = NULL, multiple = FALSE),
      checkboxInput("color_foreground", "Color Foreground Data", value = TRUE), # Corrected placement of the checkbox input
...
server <- function(input, output, session) {
  ...
  observeEvent(input$button, {
    ...
    # Create a volcano plot with foreground and background points
    volcano_plot <- ggplot() +
    ...
    # If the user wants to color the foreground data, add it to the plot
    if (input$color_foreground) {
      volcano_plot <- volcano_plot +
        geom_point(data = foreground_filtered, aes_string(x = x_col, y = paste("-log10(", y_col, ")", sep = "")), color = "gray40", size = 3, alpha = 0.6, shape = 21, fill = "gray40", stroke = 0)
    }
